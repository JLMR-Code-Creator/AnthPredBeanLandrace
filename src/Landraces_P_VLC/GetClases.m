function GetClases(pathImg, target)
    ...
    matfile = dir(strcat(pathImg,'Masks/*.mat'));% Cargar mascara de cada poblaci?n
     for k = 38:length(matfile)   
       ...
       for i2=1:size(propied,1)              % eliminaci�n de pixeles           
          ...
          text(round(propied(i2).Centroid(1)),round(propied(i2).Centroid(2)),num2str(ML(round(propied(i2).Centroid(2)),round(propied(i2).Centroid(1)))),'FontSize',8,'color','green');
       end  
       ...
       for i3=1:length(objects)
          ...
          [rawPixels, pixels] = ROILab(I_Lab, Mask_tmp)
          if strcmp(class,'Black') == 1
              ...
              save(fileName,"rawPixels","pixels","populationName","seedValue","class");
              Mask_tmp = ~Mask_tmp;
