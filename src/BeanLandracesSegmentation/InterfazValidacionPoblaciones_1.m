...
%   - Archivo principal: <nombreImagen>.mat
%   - Tambien se reconoce: <nombreImagen>_mascara.mat
...
% el reconocimiento de extensiones y el cierre seguro de la ventana.
    ...
    function cargarCarpeta(carpeta)
        ...
        setBusy(true, 'Buscando imagenes y archivos MAT...');
        ...
        for i = 1:numel(archivos)
            ...
            [rutaMat, existeMat] = localResolverRutaMat(carpeta, base);
    ...
    function aceptarGuardarSiguiente(~, ~)
        ...
        archivoTemporal = [tempname(state.Folder) '.mat'];
    ...
    function actualizarResumen()
        ...
        etiquetaResumen.Text = sprintf([ ...
            ...
            'Seleccione una fila para revisar su mascara.'], ...
            total, conMat, validadas, pendientes);
    end
    ...
    function abrirCarpetaSistema(~, ~)
        ...
        try
            if ispc
                winopen(state.Folder);
            elseif ismac
                system(sprintf('open "%s" &', state.Folder));
            else
                system(sprintf('xdg-open "%s" >/dev/null 2>&1 &', state.Folder));
            end
        catch ME
            uialert(fig, ME.message, 'No se pudo abrir la carpeta', 'Icon', 'warning');
        end
    end
...
function [rutaMat, existe] = localResolverRutaMat(carpeta, base)
% Da prioridad al MAT con el mismo nombre base que la imagen.
    rutaPrincipal = fullfile(carpeta, [base '.mat']);
    rutaAlterna = fullfile(carpeta, [base '_mascara.mat']);
