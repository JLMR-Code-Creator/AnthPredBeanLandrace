function [cie_ab, cie_la, cie_lb, pixels] = Pixel2DABLALBEscala(datos, escala)
    ...
    pixels = pixels / 256;
    %max(pixels)
    pixels = (pixels * escala) + 1;
    pixels = round(pixels);
    cie_ab=zeros(escala+1,escala+1);
    tic
    for i=1:rows
        ind = pixels(i,:);
    ...
    cie_la=zeros(escala+1,escala+1);
    tic
    for i=1:rows 
        ind = pixels(i,:);
    ...
    cie_lb=zeros(escala+1,escala+1);
    tic
    for i=1:rows 
        ind = pixels(i,:);
